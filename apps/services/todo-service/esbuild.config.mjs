 ...
 *
 * Problema que resuelve: los packages workspace `@todo-list-poc-infra/*` exponen su entrypoint
 ...
 *
 * Estrategia: bundlear SOLO los `@todo-list-poc-infra/*` (inline en dist/main.js); externalizar
  ...
  plugins: [
    {
      ...
      setup(b) {
        // Specifiers "bare" (no relativos): bundlear @todo-list-poc-infra/*, externalizar el resto
        // (npm deps + node built-ins). Los relativos los bundlea esbuild normal.
        b.onResolve({ filter: /^[^./]/ }, (args) => {
          if (args.path.startsWith("@todo-list-poc-infra/")) return null;

...
/**
 * Vista `/w/[id]/metrics` — recreada de la pantalla Stitch "Métricas y Analíticas".
 * Las stats de equipo se derivan de datos reales (miembros/roles). Burndown,
 * velocity y pronósticos de IA dependen de tasks/sprints (M2) → estado vacío honesto.
 */
export function MetricsView({
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-10">
      <WorkspaceNav workspaceId={workspace.id} active="metrics" />

      <div className="mb-8 flex flex-col items-start justify-between gap-4 lg:flex-row lg:items-center">
        <div>
          <h1 className="text-4xl font-black tracking-tight text-on-surface">Dashboard de Analítica</h1>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        {kpis.map((k) => (
          <div
            key={k.label}
            className="bouncy-hover rounded-card border border-outline-variant bg-surface p-6 text-center shadow-candy-secondary"
          >
      ...
      <section className="mt-6 rounded-card border border-outline-variant bg-surface p-8 shadow-candy-primary">
        <h2 className="mb-4 text-xl font-black text-on-surface">Distribución de roles</h2>
      ...
      {/* Burndown / Velocity → M2 (sin backend de tasks/sprints) */}
      <section className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
        {[
          { title: "Burndown del sprint", desc: "Progreso del sprint en tiempo real", icon: "📉" },
          { title: "Velocidad del equipo", desc: "Tendencia de puntos completados", icon: "🚀" },
        ].map((c) => (
          <div key={c.title} className="rounded-card border border-outline-variant bg-surface p-8 shadow-candy-tertiary">
            ...
            <div className="mt-6 flex h-40 flex-col items-center justify-center rounded-card bg-surface-container-low text-center">
              ...
              <p className="mt-2 text-sm font-bold text-on-surface-variant">
                Disponible con tableros y sprints (M2)
              </p>
            </div>
          </div>
        ))}

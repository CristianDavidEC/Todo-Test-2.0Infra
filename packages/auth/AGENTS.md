...
- **Proteger ruta NestJS** → define un Guard `@Injectable` en la app que llama `authorizeRequest(req, { usersRepo, namespace? })` y puebla `req.session`/`req.user` (patrón de `example-service`). **Leer sesión en Next** → `nextSessionToAuthSession()` (vía subpath `@todo-list-poc-infra/auth/nextjs`).

...
- Solo `zod` y `@app/types`. Para IDs usa `globalThis.crypto.randomUUID()` (Web Crypto, disponible en Node 22+/edge/browser) — **no** importa `node:crypto`, así core sigue siendo usable en cualquier runtime.
...
2. **Validación vía `@app/types`.** Core no define schemas; los consume. `validateUser`/`validateEmail` envuelven `UserSchema`; `buildEvent()` valida el envelope completo y genera el `id` (uuid) internamente.
...
- **Nueva capacidad de negocio** → función pura aquí (valida con `@app/types`). Si necesita I/O (DB, HTTP, SDK), define una **interface** aquí y su implementación **fuera** de core.

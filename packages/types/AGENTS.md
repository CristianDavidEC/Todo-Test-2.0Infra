...
# AGENTS.md — `@todo-list-poc-infra/types`
...
3. **`defineEvent(type, version, dataSchema)`** define un evento de dominio versionado: compone `DomainEventEnvelopeSchema.extend({ type: z.literal, version: z.literal, data })` y devuelve `{ type, version, schema, dataSchema } as const`. El `as const` (vía el genérico) narrowea `type` a literal; `version` queda como `string` a nivel TS — el valor exacto se valida en runtime con `z.literal(version)`. Lo consume `@todo-list-poc-infra/core/buildEvent()`.

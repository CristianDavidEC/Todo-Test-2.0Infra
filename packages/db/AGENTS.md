...
# AGENTS.md — `@todo-list-poc-infra/db`
...
3. **El schema es la fuente; migraciones generadas.** Edita `adapters/postgresql/schema.ts` y corre `pnpm --filter @todo-list-poc-infra/db db:generate` (drizzle-kit). **Nunca** escribas SQL de migración a mano. Mantén `migrations/` en historia **lineal** (conflictos de merge si no). **Aplicar** la migración es automático: el recurso `DbMigrate` (`infra/src/databases/migrate.ts`) corre `db:migrate` en cada `sst dev`/`sst deploy` de todos los stages (re-corre solo si cambia el hash de `migrations/` o el `neonDb.id`). El `db:migrate` manual queda como escape hatch.
...
- **El RBAC de la base es por claims de Auth0** (`@todo-list-poc-infra/auth` / `RolesGuard`), NO por BD. `schema.ts` solo define la tabla `users`; no hay tablas `roles`/`user_roles` (ver receta de abajo para añadirlas).
...
- **Reañadir MongoDB (datastore documental opcional):**
  1. `pnpm --filter @todo-list-poc-infra/db add mongodb`.
...
- **Reañadir RBAC por BD (roles persistidos):**
  ...
  2. `pnpm --filter @todo-list-poc-infra/db db:generate` para regenerar la migración (historia lineal).
  3. Crear el/los `*.repository.ts` (clase) y exportarlos en `index.ts`. Cablear el RBAC contra BD en `@todo-list-poc-infra/auth`/los guards (hoy el RBAC es claim-based de Auth0).

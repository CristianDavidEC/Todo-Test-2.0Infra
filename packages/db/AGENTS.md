...
3. **El schema es la fuente; migraciones generadas.** Edita `adapters/postgresql/schema.ts` y corre `pnpm --filter @todo-list-poc-infra/db db:generate` (drizzle-kit). **Nunca** escribas SQL de migración a mano. Mantén `migrations/` en historia **lineal** (conflictos de merge si no).
